...
gm.mysql.handle.query('SELECT * FROM server_rent_vehicles', [], function (error, results, fields) {
	for(let i = 0; i < results.length; i++) {
        ...
        struct.rentCar[i].rentModelID = results[i].rentID;

        struct.rentCar[i].rentModelName = results[i].rentModel;
        struct.rentCar[i].rentModelPrice = results[i].rentPrice;
		struct.rentCar[i].rentModelStock = results[i].rentStock;
        
        loaded_rentCar_count ++;
    }
    
    console.log(`[MYSQL] Loaded rent vehicles: ${loaded_rentCar_count.toString()}`);
});
...
mp.events.addCommand('rentcar', (player) => {
    ...
    for(let x = 0; x < loaded_rentCar_count; x++)
    { 
        rentString += `<tr><td>${x + 1}</td><td>${struct.rentCar[x].rentModelName}</td><td>${player.formatMoney(struct.rentCar[x].rentModelPrice, 0)}$</td><td>${struct.rentCar[x].rentModelStock}</td><td><button class="btn btn-success btn-sm" id = "rentAVehicle" onclick = "sendRentInfo(${struct.rentCar[x].rentModelID});">Rent</button></td> </tr>`;
    }
...
mp.events.add('playerPressRentButton', (player, type) => {
    ...
    if(struct.rentCar[x].rentModelPrice > player.data.money)
    ...
    player.giveMoney(1, struct.rentCar[x].rentModelPrice);
  
    player.data.rentedVehicle = player.createVehicle(player, struct.rentCar[x].rentModelName, new mp.Vector3(spawn[0], spawn[1], spawn[2]), generateRGB(), generateRGB(), spawn[3], 1);
  
    player.call("closePlayerRentBrowser");
 
    struct.rentCar[x].rentModelStock = struct.rentCar[x].rentModelStock - parseInt(1);
    mysql_action('UPDATE `server_rent_vehicles` SET rentStock = ? WHERE rentID = ? LIMIT 1', [struct.rentCar[x].rentModelStock, struct.rentCar[x].rentModelID]);

	sendMessage(player, 'FFFFFF', `------------------!{ff4d4d}(Rent vehicle):!{ffffff}----------------`);
	sendMessage(player, 'FFFFFF', `Vehicle rented this !{ff4d4d}${struct.rentCar[x].rentModelName}!{ffffff} for !{ff4d4d}${player.formatMoney(struct.rentCar[x].rentModelPrice, 0)}!{ffffff}$`);
