...
global.loaded_rentCar_count = 0;
global.loaded_rentBike_count = 0;
...
{   
	if(player.data.mute > 0) 
	    return sendMessage(player, 'ffffff', `You are muted, you cannot speak.`);
...
{  
    ...
    if((player.data.drivingLicense == 0 && seat == -1) && player.data.InDMV == 0 && player.getVariable('bikeRentedTime') == -1)
    {
...
{ 
    ...
    if((player.data.drivingLicense == 0 && seat == 0) && player.data.InDMV == 0 && player.getVariable('bikeRentedTime') == -1)
    {
		player.removeFromVehicle();
		player.stopAnimation();
        sendMessage(player, 'ff4d4d', `(License):!{ffffff} You don't have driving license.`);
